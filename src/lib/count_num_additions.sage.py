...
# This file was *autogenerated* from the file src/lib/count_num_additions.sage
from sage.all_cmdline import *   # import sage library

_sage_const_3 = Integer(3); _sage_const_100000 = Integer(100000); _sage_const_10 = Integer(10); _sage_const_0 = Integer(0); _sage_const_1 = Integer(1); _sage_const_0p99 = RealNumber('0.99'); _sage_const_0p0 = RealNumber('0.0'); _sage_const_1p0 = RealNumber('1.0'); _sage_const_32003 = Integer(32003); _sage_const_1000000 = Integer(1000000); _sage_const_100 = Integer(100)
import bisect
import numpy as np
...
class NumAdditionsCounter:
    def __init__(self, coeff_field, num_variables=_sage_const_3 , select_strategy='normal', threshold=_sage_const_100000 , timeout=_sage_const_10 , check_gb=False):
    ...
    def run(self, weight_matrix, polynomial_list, variable_order=None):
        ...
        polynomial_list = [p for p in polynomial_list if p != _sage_const_0 ]
    ...
    def str_to_matrix(self, str_term_order):
        ...
        if str_term_order == 'lex':
            ...
            first_row = np.ones((_sage_const_1 , self.num_variables), dtype=int)
            rows = np.eye(nvars, dtype=int)

            if str_term_order == 'grlex':
                weight_matrix = np.concatenate([first_row, rows])
            else: # grevlex
                rows = np.flip(np.eye(nvars, dtype=int), axis=_sage_const_1 )
                rows = -rows
...
class OutputContent:
    def __init__(self, gb, zero_reductions, nonzero_reductions, polynomial_additions, mono_div_steps, total_reward, discounted_return, success, is_gb, total_time, reduction_time, selection_time, polynomial_list, weight_matrix, variable_order):
        self.gb = gb
...
class BuchbergerAlgorithm:
    def __init__(self, Ring, polynomial_list, S=None, select_strategy='degree', threshold=None, timeout=None, check_gb=False, elimination='gebauermoeller', rewards='additions', sort_reducers=True, gamma=_sage_const_0p99 ):
        self.Ring = Ring
        self.polynomial_list = polynomial_list
        self.S = S
        self.select_strategy = select_strategy
        ...
        self.gamma = gamma
        self.timeout = timeout
        self.start_time = _sage_const_0p0 
        self.current_time = _sage_const_0p0 
        self.check_gb = check_gb

        self.stats = {'zero_reductions': _sage_const_0 ,
                'nonzero_reductions': _sage_const_0 ,
                'polynomial_additions': _sage_const_0 ,
                'mono_div_steps': _sage_const_0 ,
                'total_reward': _sage_const_0p0 ,
                'discounted_return': _sage_const_0p0 ,
                'total_time': _sage_const_0p0 ,
                'reduction_time': _sage_const_0p0 ,
                'selection_time': _sage_const_0p0 ,
                'success': None,
                'is_gb': None,
                'polynomial_list': self.polynomial_list,
                'weight_matrix': np.array(self.Ring.term_order().matrix()).reshape(-_sage_const_1 ),
    ...
    def __call__(self):
        ...
        if self.sort_reducers and len(G) > _sage_const_0 :
            G_ = G.copy()
        ...
        while P:
            ...
            self.stats['polynomial_additions'] += _sage_const_1 
            
            r, s = self.reduce(spoly_, G_, interrupt_flag=True)

            reward = (-_sage_const_1p0  - s['steps']) if self.rewards == 'additions' else -_sage_const_1p0 
            ...
            if r != _sage_const_0 :
                ...
                if self.sort_reducers:
                    ...
                    G_ = G
                    lmG_ = lmG
                self.stats['nonzero_reductions'] += _sage_const_1 
            else:
                self.stats['zero_reductions'] += _sage_const_1 
    ...
    def reduce(self, g, F, lmF=None, interrupt_flag=False):
        ...
        stats = {'steps': _sage_const_0 , 'mono_div_steps': _sage_const_0 }
        ...
        while h != _sage_const_0 :
            ...
            for f, lmf in zip(F, lmF):
                ...
                stats['mono_div_steps'] += _sage_const_1 
    ...
    def minimalize(self, G):
        ...
        Ring = self.Ring if len(G) > _sage_const_0  else None
        Gmin = []
    ...
    def interreduce(self, G):
        ...
        for i in range(len(G)):
            g, _ = self.reduce(G[i], G[:i] + G[i+_sage_const_1 :])
...
if __name__ == '__main__':
    nvars = _sage_const_10 
    field = GF(_sage_const_32003 )
    ...
    print('reduction time rate:', result.reduction_time / result.total_time * _sage_const_100 )
    print('selection time rate:', result.selection_time / result.total_time * _sage_const_100 )
