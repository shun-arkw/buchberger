...
# This file was *autogenerated* from the file src/diff_evo.sage
from sage.all_cmdline import *   # import sage library

_sage_const_0 = Integer(0); _sage_const_3 = Integer(3); _sage_const_10000 = Integer(10000); _sage_const_5 = Integer(5); _sage_const_1e10 = RealNumber('1e10'); _sage_const_2 = Integer(2); _sage_const_1 = Integer(1); _sage_const_1000 = Integer(1000); _sage_const_15 = Integer(15); _sage_const_10 = Integer(10); _sage_const_100000 = Integer(100000); _sage_const_60 = Integer(60)
import numpy as np
import pprint
import argparse
import os
...
def func(params, data, coeff_field, operation='mean', num_variables=_sage_const_3 , select_strategy='normal', threshold=_sage_const_10000 , timeout=_sage_const_5 ):
    INF = _sage_const_1e10 

    assert len(params) == num_variables**_sage_const_2 , "The number of parameters must be equal to the number of entries in the weight matrix."
    ...
    ret = Parallel(n_jobs=-_sage_const_1 , backend="multiprocessing", verbose=True)(delayed(num_additions_counter.run)(weight_matrix=weight_matrix, polynomial_list=F) for F in data)
...
def get_parser():
    ...
    parser.add_argument("--nvars", type=int, default=_sage_const_3 , help="変数の数")
    parser.add_argument("--field_name", type=str, default='GF7', help="体の名前")
    parser.add_argument("--dataset_size", type=int, default=_sage_const_1000 , help="多項式系の数")
    parser.add_argument("--upper_bound", type=int, default=_sage_const_1000 , help="重み行列の成分の上限値")
    parser.add_argument("--lower_bound", type=int, default=-_sage_const_1000 , help="重み行列の成分の下限値")
    parser.add_argument("--diff_evo_popsize", type=int, default=_sage_const_15 , help="進化計算でのpopsize")
    parser.add_argument("--diff_evo_maxiter", type=int, default=_sage_const_1000 , help="maxiter")
    parser.add_argument("--diff_evo_workers", type=int, default=_sage_const_1 , help="進化計算での並列数")
    ...
    parser.add_argument("--buchberger_timeout", type=int, default=_sage_const_10 , help="buchbergerアルゴリズムのタイムアウト")
    parser.add_argument("--buchberger_threshold", type=int, default=_sage_const_100000 , help="buchbergerアルゴリズムの打ち切り閾値(多項式加算数)")
...
if __name__ == '__main__':
    ...
    bounds = [(_sage_const_0 , upper)] * nvars + [(lower, upper)] * (nvars ** _sage_const_2  - nvars)
    ...
    time_h = int(elapsed_time / (_sage_const_60  * _sage_const_60 ))
    time_m = int((elapsed_time % (_sage_const_60  * _sage_const_60 )) / _sage_const_60 )
    time_s = int(elapsed_time % _sage_const_60 )
    ...
    # f.write(str(result) + '\n\n')
    f.write('population size: ' + str(result.population.shape[_sage_const_0 ]) + '\n')
